...
export default function Sidebar({ agentName, agentEmail }: SidebarProps) {
  ...
  return (
    <aside className="w-60 shrink-0 bg-gray-900 flex flex-col h-screen sticky top-0 overflow-hidden">
      ...
      <div className="px-3 pb-4 border-t border-gray-800 pt-4 space-y-1">
        <div className="flex items-center gap-3 px-3 py-2">
          <div className="w-8 h-8 bg-indigo-700 rounded-full flex items-center justify-center shrink-0">
            <span className="text-white text-xs font-semibold">{initials(agentName)}</span>
          </div>
          <div className="min-w-0 flex-1">
            <div className="text-white text-sm font-medium truncate">{agentName}</div>
            <div className="text-gray-500 text-xs truncate">{agentEmail}</div>
          </div>
        </div>
        <button
